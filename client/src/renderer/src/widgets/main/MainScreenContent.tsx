...
function getPrimaryActionLabel(phase: string, installedVersion: string | null) {
  if (phase === 'failed') {
...
export function MainScreenContent() {
  ...
  const installBusy = installState.phase === 'downloading' || installState.phase === 'extracting' || installState.phase === 'checking';
  const playDisabled = Boolean(activeBan) || installState.phase !== 'ready' || dataLoading;
  const playLabel = activeBan ? 'Play blocked by ban' : installState.phase === 'ready' ? 'Play (runtime wiring next)' : 'Play locked until build is ready';
  const installActionLabel = getPrimaryActionLabel(installState.phase, installState.installedVersion);
  ...
  const logLines = useMemo(
    ...
    [activeBan, activeRelease?.version, installState.installedVersion, installState.message, installState.phase, news.length, roleLabel, session?.user.email],
  );
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-2">
      <section className="xl:col-span-2 rounded-[28px] border border-white/10 bg-[linear-gradient(135deg,rgba(255,255,255,0.08),rgba(255,255,255,0.02))] p-8 shadow-[0_24px_80px_rgba(0,0,0,0.45)] backdrop-blur-xl">
        <div className="space-y-4">
          ...
          <p className="max-w-3xl text-base leading-7 text-slate-300">
            This screen now reflects live auth, release metadata, and the first real build install/update flow. Minecraft runtime launch stays for the next phase, but install readiness, retry handling, bans, and release status are already live.
          </p>
        </div>
        <div className="mt-6 flex flex-wrap items-center gap-3">
          ...
          <Button
            type="button"
            disabled={installBusy || !activeRelease || !activeRelease.zip_url || installState.phase === 'ready'}
            onClick={() => void installActiveRelease()}
          >
            {installActionLabel}
          </Button>
          {installState.phase === 'failed' ? (
            <Button variant="secondary" type="button" onClick={() => void retryInstall()}>
              Retry install
            </Button>
          ) : null}
          <Button variant="secondary" type="button" onClick={() => void refreshInstallState()}>
          ...
          <StatusBadge label={installState.phase === 'ready' ? 'Build ready' : installState.phase === 'failed' ? 'Build failed' : installState.updateAvailable ? 'Build update available' : 'Build pending'} tone={installState.phase === 'ready' ? 'live' : installState.phase === 'failed' ? 'warn' : 'muted'} />
        ...
        {installState.lastError ? <div className="mt-4 rounded-2xl border border-amber-300/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-100">{installState.lastError}</div> : null}
      </section>
      ...
      <Panel title="Install status" kicker="Local vs remote build">
        <ul className="m-0 space-y-3 pl-5 text-sm leading-7 text-slate-300">
          <li>Remote active version: {activeRelease?.version ?? 'No active release published yet'}</li>
          <li>Installed version: {installState.installedVersion ?? 'Nothing installed locally'}</li>
      ...
      <Panel title="Play access" kicker="Ban and release state">
        <ul className="m-0 space-y-3 pl-5 text-sm leading-7 text-slate-300">
          ...
          <li>Play gate: {playDisabled ? 'blocked' : 'ready'}</li>
        </ul>
      </Panel>
      ...
      <Panel title="Version status" kicker="Release metadata">
        <ul className="m-0 space-y-3 pl-5 text-sm leading-7 text-slate-300">
          ...
          <li>ZIP URL: {activeRelease?.zip_url ?? 'not provided'}</li>
